...
(check-expect (draw-ds ds1) (overlay/xy PHOTOG
                            3
                            (- HEIGHT PHOTOG-HEIGHT)
                            (overlay/xy (goal (make-interval 12 24))
                                        12
                                        (- HEIGHT INTERVAL-HEIGHT)
                                        SCENE)))
(check-expect (draw-ds ds3) (rectangle 12 20 "solid" "green"))


; goal : Interval -> Image
; Creates a visual representation of the goal described by the interval.
(define (goal interval)
  (rectangle (- (interval-right interval) (interval-left interval)) INTERVAL-HEIGHT "solid" "gold"))
...
;; WRITER RESTSETSEASALTjrsahjgkqbda sga
